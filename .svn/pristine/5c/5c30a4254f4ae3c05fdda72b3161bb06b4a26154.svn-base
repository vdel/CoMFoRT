...
def verifie(cond,record):
  for c in cond:
    if parse_condition(cond[1],record[cond[0]],cond[2]):
      return 1
  return 0
  
def verifie_conditions(conditions,record):
  if conditions == []:
    return 1
  else:
    a = verifie(conditions[0],record)
    b = verifie_conditions(conditions[1:],record)
    c = a & b # a optimiser ## to be improved
    return c
    # optimisation future
    ## future optimisation
    # a = ....
